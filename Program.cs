namespace kontorNord_isa_dokumentation
{
		...
		namespace ProjektNordUgevisning_Aflysning_af_møde_isa
		{
			...
			internal class Program
			{
				...
				static void Main(string[] args)
				{
					...
					Console.Title = "♥ Bookingsystem - KontorNord ♥"; // Titel på konsol-vinduet :)
					Console.ResetColor();
					...
					List<MeetingRoom> rooms = new List<MeetingRoom> // Vi opretter tre mødelokaler med forskellige kapaciteter. Id bruges til at knytte møder til lokaler, Name er det brugeren ser i menuen og Capacity kan bruges i fremtiden hvis vi vil implementere en funktion der tjekker om lokalet har plads til det antal deltagere der er i mødet.
					{
					...
					int selectedRoomIndex = 0; // hvilket lokale der aktuelt er valgt i visningen. Vi starter fra første lokale i listen.
							...
							case 2:
								{
									...
									// Tjek om lokalet er ledigt i det ønskede tidsrum før vi gemmer mødet. Havde evt været bedre at tjekke tidligere i booking-flow.
									// Samme tidspunkt kan godt findes i andre lokaler, men ikke i det lokale brugeren står på.
									if (!selectedRoom.IsAvailable(meetings, newMeeting.IsoYear, newMeeting.IsoWeek, newMeeting.IsoDay, newMeeting.StartHour, newMeeting.EndHour))
									{
										Console.WriteLine();
										Console.ForegroundColor = ConsoleColor.Red;
										Console.WriteLine("Lokalet er ikke ledigt i det tidsrum. Tryk på en tast...");
										Console.ResetColor();
									...
									if (isConfirmed)
									{
										...
										Console.WriteLine();
										Console.WriteLine("Mødet er nu oprettet.");// Denne besked bliver heller ikke vist i det nuværende flow.
